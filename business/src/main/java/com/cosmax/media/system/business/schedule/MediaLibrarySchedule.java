package com.cosmax.media.system.business.schedule;
 ...
 * @description: 媒体库表自动任务
 * @author: Cosmax
 * @create: 2020/03/03 18:39
 */
@Component
public class MediaLibrarySchedule {
    ...
    public void autoSaveOrUpdateMedia(){
        ...
        List<MediaLibrary> mediaLibraries = mediaLibraryService.getScheduleList(com.cosmax.media.system.commons.domain.douban.MediaLibrarySchedule.SAVE_STATE);
        ...
        for (MediaLibrary mediaLibrary : mediaLibraries) {
            ...
            // 删除媒体库目录 硬链接目录--已废除
//            FileUtils.deleteQuietly(new File(FileUtils.getJarFilePath() + "/videos/" + mediaLibrary.getId()));

            // 删除原来信息
            initDatabase(mediaLibrary.getId(), mediaLibrary.getType());


            String directories = mediaLibrary.getDirectory();  // 文件目录
            if (mediaLibrary.getType().equals(SubTypeEnum.MOVIE.getId())){
                // 电影类型

                // 通过媒体库路径获取List<MovieInfo>
                List<MovieInfo> movieInfosFromPath = getMovieInfoByDirectories(directories);
...
//                        LoggerUtils.sendMessage(getClass(), LoggerUtils.WARN_LOGGER, "电影[" + fileName +"]创建硬链接失败!", Thread.currentThread().getStackTrace()[1].getMethodName(), 1);
//                        continue;
//                    }
//                    movieInfo.setLinkPath(movieInfo.getPath());

                    // 写入数据库
                    // 写入media_video_douban和movie表
                    long movieId = System.currentTimeMillis();  // 生成id
                ...
                // 设置任务表is_success标识为1
                updateScheduleIsSuccess(mediaLibrary.getId(), com.cosmax.media.system.commons.domain.douban.MediaLibrarySchedule.SAVE_STATE, SuccessEnum.SUCCESS.getId());

            }
            if (mediaLibrary.getType().equals(SubTypeEnum.TV.getId())){
                // 剧集类型
                // 获取List<SeriesInfo>
                List<SeriesInfo> seriesInfosFormPath = getSeriesInfosByDirectories(directories);
                ...
                for (SeriesInfo seriesInfo : seriesInfosFormPath) {
                    ...
                    for (EpisodeInfo episodeInfo : episodeInfos) {
                        ...
                        // 硬链接--已废除
//                        String fileName = seriesInfo.getName() + "." + MediaFileUtils.getSeasonEpisode(seriesInfo.getSeason(), episodeInfo.getEpisode()) + "." + episodeInfo.getSuffix();
//                        String link = FileUtils.createLink(episodeInfo.getPath(), mediaLibrary.getId(), fileName);
//                        if (StringUtils.isBlank(link)){
//                            LoggerUtils.sendMessage(getClass(), LoggerUtils.WARN_LOGGER, "视频["+mediaLibrary.getId()+ "---" + seriesId + "---" + fileName + "]创建硬链接失败!", Thread.currentThread().getStackTrace()[1].getMethodName(), 5);
//                            continue;
//                        }
//                        episodeInfo.setLinkPath(link);
                        ...
                        seriesEpisode.setEpisodeLocation(episodeInfo.getPath());
    ...
    /**
     * 根据目录字符串获取剧集信息list
     * @param directories 目录字符串，一个或者用|隔开的多个
     * @return 剧集信息list
     */
    private List<SeriesInfo> getSeriesInfosByDirectories(String directories) {
     ...
     * @return 电影信息list
     */
    private List<MovieInfo> getMovieInfoByDirectories(String directories) {
    ...
    public void autoDeleteMedia(){
        ...
        List<MediaLibrary> mediaLibraries = mediaLibraryService.getScheduleList(com.cosmax.media.system.commons.domain.douban.MediaLibrarySchedule.DELETE_STATE);
        if (mediaLibraries == null || mediaLibraries.size() <= 0){
            return;
        }
        for (MediaLibrary mediaLibrary : mediaLibraries) {
...
//                updateScheduleIsSuccess(id, com.cosmax.media.system.commons.domain.douban.MediaLibrarySchedule.DELETE_STATE, SuccessEnum.EXCEPTION.getId());
//                continue;
//            }

            // 修改状态
            updateScheduleIsSuccess(id, com.cosmax.media.system.commons.domain.douban.MediaLibrarySchedule.DELETE_STATE, SuccessEnum.SUCCESS.getId());
        }
    ...
    private void addObserver(MediaLibrary mediaLibrary) {
        for (FileAlterationObserver observer : monitor.getObservers()) {
            for (FileAlterationListener listener : observer.getListeners()) {
                VideoFileListener v = (VideoFileListener) listener;
                if (mediaLibrary.equals(v.getMediaLibrary())){
                    return;
                }
            }
        }
        ...
        for (DoubanVideoInfo info : infos) {
            ...
            if (type.equals(SubTypeEnum.MOVIE.getId())){
                ...
                if (one != null){
                    ...
                    boolean updateMediaVideoDouban = mediaVideoDoubanService.update(mediaVideoDouban, new UpdateWrapper<MediaVideoDouban>().lambda().eq(MediaVideoDouban::getMediaId, mediaId).eq(MediaVideoDouban::getItemId, one.getMovieId()));
                    if (!updateMediaVideoDouban){
                        LoggerUtils.sendMessage(getClass(), LoggerUtils.WARN_LOGGER, mediaId + "--- " + videoName + "---绑定媒体库豆瓣视频失败！", Thread.currentThread().getStackTrace()[1].getMethodName(), 1);
                    }else {
                        LoggerUtils.sendMessage(getClass(), LoggerUtils.INFO_LOGGER, mediaId + "--- " + videoName +"---绑定媒体库豆瓣视频成功！", Thread.currentThread().getStackTrace()[1].getMethodName(), 1);
                    }
                }
            }
            if (type.equals(SubTypeEnum.TV.getId())){
                ...
                if (seriesInfo != null){
                    ...
                    mediaVideoDouban.setDoubanId(info.getDoubanInfo().getDoubanId());
                    boolean updateMediaVideoDouban = mediaVideoDoubanService.update(mediaVideoDouban, new UpdateWrapper<MediaVideoDouban>()
                            .lambda()
                            .eq(MediaVideoDouban::getMediaId, mediaId).eq(MediaVideoDouban::getItemId, seriesInfo.getSeriesId()));
                    if (!updateMediaVideoDouban){
                        LoggerUtils.sendMessage(getClass(), LoggerUtils.WARN_LOGGER, mediaId + "--- " + videoName + "---绑定媒体库豆瓣视频失败！", Thread.currentThread().getStackTrace()[1].getMethodName(), 1);
                    }else {
                        LoggerUtils.sendMessage(getClass(), LoggerUtils.INFO_LOGGER, mediaId + "--- " + videoName +"---绑定媒体库豆瓣视频成功！", Thread.currentThread().getStackTrace()[1].getMethodName(), 1);
